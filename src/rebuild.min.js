(function (window, document) {
    ...
    function contentParse(content = '') {
        return content.replace($CONTENT_REGEX, (_match, key) => {
            ...
            let newKey = key;
            if (CALLBACK_REGEX.test(key)) {
                const funName = key.match(FUN_NAME_REGEX)[0];
                const matches = [...key.matchAll(/\(\s*([^()]*)\s*\)/g)];
                const params = matches.map(m => m[1]);
                const newParams = params[0].split(',').map(param => {
                    if (STR_REGEX.test(param.trim())) return param
                    return 'this.' + param.trim()
                });
                newKey = `${funName}(${newParams})`
            }
