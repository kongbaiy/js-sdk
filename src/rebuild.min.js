(function (window, document) {
    ...
    function classParse(tag, classContent) {
        const classValue = contentParse(classContent.match($CONTENT_REGEX)?.[0]);
        return tag.replace('>', ' class="' + classContent.replace($CONTENT_REGEX, classValue) +'">')
    }
    ...
    function compose(tags) {
        ...
        for (let t of tags) {
            ...
            if (type === 'node') {
                ...
                if (t.$style) {
                    t.tag = styleParse(t.tag, t.$style)
                } 

                if (t.$class) {
                   t.tag = classParse(t.tag, t.$class)
                } 
