(function (window, document) {
    ...
    function resolver(htmlStr) {
        ...
        while ((match = TAG_REGEX.exec(htmlStr)) !== null) {
            ...
            if (tag.startsWith('</')) {
                ...
                if (styleMatch) item.$style = styleMatch.replace('&gt;', '>').replace('&lt;', '<').replaceAll('&amp;', '&');
    ...
    //     const [start, styleValue, end] = style.split(/\{\{\s*(.*?)\s*\}\}/);
    //     return '`' + start.replace('v-style', 'style') + '${' + styleValue + '}' + end + '`';
    // }

    function styleParse(tag, styleContent) {
        const styleValue = contentParse(styleContent.match($CONTENT_REGEX)[0]);
        return tag.replace('>', ' style="' + styleContent.replace($CONTENT_REGEX, styleValue) +'">')
    }
    ...
    function compose(tags) {
        ...
        for (let t of tags) {
            ...
            if (type === 'node') {
                ...
                if (t.$style) {
                    newTag = styleParse(t.tag, t.$style)
                } 
